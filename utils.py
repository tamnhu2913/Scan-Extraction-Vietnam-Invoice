...
class ExtractorInfor:
    ...
    def extractNameDate(info):
        extrct = {'Ten': '', 'Ngay': ''}
        keywords = ["siêu thị", "cửa hàng", "mart", "food", "market", "nhà hàng", "coffee", "cafe", "cà phê"]
        regex1 = '|'.join(keywords)

        regex2 = r"[ạảãàáâậầấẩẫăắằặẳẵóòọõỏôộổỗồốơờớợởỡéèẻẹẽêếềệểễúùụủũưựữửừứíìịỉĩýỳỷỵỹđ\d{4},]"
        extrct['Ten'] = next(
    ...
    def extractTotalSale(total):
        ...
        for sub in total:
            ...
            if re.search(r'vat|thue', ' '.join(sub), re.IGNORECASE) and t:
                continue
            elif re.search(r'giam|giảm', ' '.join(sub), re.IGNORECASE) and t:
                km = re.findall(regex2, ' '.join(sub))[0]
            elif len(money) < 2 and t:
                money.append(int(re.sub(r'[^\d]', '', t[0])))

        if money and len(money) in range(1,3):
